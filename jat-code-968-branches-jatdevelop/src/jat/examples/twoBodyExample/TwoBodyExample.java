...
public class TwoBodyExample implements Printable
{
    ...
    public static void main(String[] args)
    {
        ...
        TwoBodyAPL sat = new TwoBodyAPL(7000.0, 0.5, 0.0, 0.0, 0.0, 0.0);
        ...
        // propagate the orbit
        sat.propagate(t0, tf/6, x, true);

		x.traj_plot.plot.addPoint(1, 100., 100., true);
		x.traj_plot.plot.addPoint(1, 200., 100., false);

		x.traj_plot.plot.setMarksStyle("dots");

		VectorN newpos;
		
        sat.propagate(t0, tf/4, x, false);
        newpos= sat.getR();
		x.traj_plot.plot.addPoint(2, newpos.x[0], newpos.x[1], true);
        
        sat.propagate(t0, tf/2, x, false);
        newpos= sat.getR();
		x.traj_plot.plot.addPoint(3, newpos.x[0], newpos.x[1], true);
